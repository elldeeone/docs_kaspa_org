# Fact-Check Master Summary

- Total docs audited: **146**
- `ISSUES`: **104**
- `UNVERIFIED`: **33**
- `VERIFIED`: **9**

## Coverage
- Sources audited: all `content/docs/**/*.mdx` plus `README.md`.
- Coverage gap count: **0**

## Status By File
| File | Status |
|---|---|
| `README.md` | `ISSUES` |
| `content/docs/cookbook/node/check-node-status.mdx` | `ISSUES` |
| `content/docs/cookbook/node/connect-grpc.mdx` | `ISSUES` |
| `content/docs/cookbook/node/connect-wrpc.mdx` | `ISSUES` |
| `content/docs/cookbook/node/query-utxos.mdx` | `ISSUES` |
| `content/docs/cookbook/node/subscribe-blocks.mdx` | `ISSUES` |
| `content/docs/cookbook/transactions/batch-transactions.mdx` | `UNVERIFIED` |
| `content/docs/cookbook/transactions/estimate-fees.mdx` | `UNVERIFIED` |
| `content/docs/cookbook/transactions/send-kas.mdx` | `VERIFIED` |
| `content/docs/cookbook/transactions/track-confirmation.mdx` | `ISSUES` |
| `content/docs/cookbook/transactions/utxo-consolidation.mdx` | `UNVERIFIED` |
| `content/docs/cookbook/wallets/create-wallet.mdx` | `VERIFIED` |
| `content/docs/cookbook/wallets/generate-address.mdx` | `ISSUES` |
| `content/docs/cookbook/wallets/get-balance.mdx` | `ISSUES` |
| `content/docs/cookbook/wallets/restore-from-mnemonic.mdx` | `ISSUES` |
| `content/docs/cookbook/wallets/sign-message.mdx` | `ISSUES` |
| `content/docs/core-concepts/addresses.mdx` | `ISSUES` |
| `content/docs/core-concepts/blockdag.mdx` | `UNVERIFIED` |
| `content/docs/core-concepts/dagknight.mdx` | `ISSUES` |
| `content/docs/core-concepts/difficulty-adjustment.mdx` | `ISSUES` |
| `content/docs/core-concepts/emission-schedule.mdx` | `ISSUES` |
| `content/docs/core-concepts/fees-and-mass.mdx` | `ISSUES` |
| `content/docs/core-concepts/ghostdag.mdx` | `UNVERIFIED` |
| `content/docs/core-concepts/pruning.mdx` | `ISSUES` |
| `content/docs/core-concepts/scripting.mdx` | `ISSUES` |
| `content/docs/core-concepts/spv.mdx` | `UNVERIFIED` |
| `content/docs/core-concepts/transactions.mdx` | `ISSUES` |
| `content/docs/core-concepts/utxo-model.mdx` | `UNVERIFIED` |
| `content/docs/developer-guide/building-apps.mdx` | `ISSUES` |
| `content/docs/developer-guide/integration.mdx` | `ISSUES` |
| `content/docs/getting-started/first-transaction.mdx` | `ISSUES` |
| `content/docs/getting-started/installation.mdx` | `ISSUES` |
| `content/docs/getting-started/quick-start.mdx` | `ISSUES` |
| `content/docs/getting-started/what-is-kaspa.mdx` | `ISSUES` |
| `content/docs/getting-started/why-kaspa.mdx` | `ISSUES` |
| `content/docs/guides/build-payment-gateway.mdx` | `UNVERIFIED` |
| `content/docs/guides/build-wallet-app.mdx` | `UNVERIFIED` |
| `content/docs/guides/dev-environment.mdx` | `ISSUES` |
| `content/docs/guides/first-transaction.mdx` | `UNVERIFIED` |
| `content/docs/guides/kaspad-to-rusty-kaspa.mdx` | `UNVERIFIED` |
| `content/docs/guides/monitoring-setup.mdx` | `UNVERIFIED` |
| `content/docs/guides/production-node.mdx` | `ISSUES` |
| `content/docs/guides/testnet-development.mdx` | `ISSUES` |
| `content/docs/index.mdx` | `UNVERIFIED` |
| `content/docs/integrations/block-explorers.mdx` | `UNVERIFIED` |
| `content/docs/integrations/exchange-integration.mdx` | `ISSUES` |
| `content/docs/integrations/payment-processing.mdx` | `UNVERIFIED` |
| `content/docs/integrations/third-party-protocols.mdx` | `UNVERIFIED` |
| `content/docs/mining/block-rewards.mdx` | `ISSUES` |
| `content/docs/mining/difficulty-adjustment.mdx` | `ISSUES` |
| `content/docs/mining/hardware.mdx` | `UNVERIFIED` |
| `content/docs/mining/kheavyhash.mdx` | `ISSUES` |
| `content/docs/mining/pool-mining.mdx` | `UNVERIFIED` |
| `content/docs/mining/solo-mining.mdx` | `ISSUES` |
| `content/docs/node-operations/configuration.mdx` | `ISSUES` |
| `content/docs/node-operations/docker.mdx` | `ISSUES` |
| `content/docs/node-operations/monitoring.mdx` | `ISSUES` |
| `content/docs/node-operations/running-a-node.mdx` | `ISSUES` |
| `content/docs/node-operations/simnet.mdx` | `ISSUES` |
| `content/docs/node-operations/testnet.mdx` | `ISSUES` |
| `content/docs/node-operations/troubleshooting.mdx` | `UNVERIFIED` |
| `content/docs/node-operations/upgrading.mdx` | `UNVERIFIED` |
| `content/docs/protocol-evolution/crescendo.mdx` | `UNVERIFIED` |
| `content/docs/protocol-evolution/kip-index.mdx` | `ISSUES` |
| `content/docs/protocol-evolution/kips-overview.mdx` | `UNVERIFIED` |
| `content/docs/protocol-evolution/roadmap.mdx` | `UNVERIFIED` |
| `content/docs/reference/api-reference.mdx` | `ISSUES` |
| `content/docs/reference/emission-schedule.mdx` | `UNVERIFIED` |
| `content/docs/reference/glossary.mdx` | `ISSUES` |
| `content/docs/reference/network-params.mdx` | `ISSUES` |
| `content/docs/reference/research-papers.mdx` | `ISSUES` |
| `content/docs/reference/specifications.mdx` | `ISSUES` |
| `content/docs/rpc/connecting.mdx` | `ISSUES` |
| `content/docs/rpc/grpc/overview.mdx` | `ISSUES` |
| `content/docs/rpc/grpc/protobuf-schema.mdx` | `ISSUES` |
| `content/docs/rpc/grpc/streaming.mdx` | `ISSUES` |
| `content/docs/rpc/overview.mdx` | `ISSUES` |
| `content/docs/rpc/rest/addresses.mdx` | `ISSUES` |
| `content/docs/rpc/rest/blocks.mdx` | `ISSUES` |
| `content/docs/rpc/rest/network-info.mdx` | `ISSUES` |
| `content/docs/rpc/rest/transactions.mdx` | `ISSUES` |
| `content/docs/rpc/rest/virtual-chain.mdx` | `ISSUES` |
| `content/docs/rpc/subscriptions/block-added.mdx` | `VERIFIED` |
| `content/docs/rpc/subscriptions/finality-conflict.mdx` | `ISSUES` |
| `content/docs/rpc/subscriptions/new-block-template.mdx` | `ISSUES` |
| `content/docs/rpc/subscriptions/overview.mdx` | `ISSUES` |
| `content/docs/rpc/subscriptions/pruning-point-utxo-set-override.mdx` | `ISSUES` |
| `content/docs/rpc/subscriptions/sink-blue-score-changed.mdx` | `ISSUES` |
| `content/docs/rpc/subscriptions/utxos-changed.mdx` | `VERIFIED` |
| `content/docs/rpc/subscriptions/virtual-chain-changed.mdx` | `ISSUES` |
| `content/docs/rpc/subscriptions/virtual-daa-score-changed.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/add-peer.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/ban.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/estimate-network-hashes-per-second.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-balance-by-address.mdx` | `VERIFIED` |
| `content/docs/rpc/wrpc/get-balance-by-addresses.mdx` | `VERIFIED` |
| `content/docs/rpc/wrpc/get-block-count.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-block-dag-info.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-block-template.mdx` | `UNVERIFIED` |
| `content/docs/rpc/wrpc/get-block.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-blocks.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-coin-supply.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-connected-peer-info.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-connections.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-current-block-color.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-current-network.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-daa-score-timestamp-estimate.mdx` | `VERIFIED` |
| `content/docs/rpc/wrpc/get-fee-estimate-experimental.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-fee-estimate.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-headers.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-info.mdx` | `VERIFIED` |
| `content/docs/rpc/wrpc/get-mempool-entries-by-addresses.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-mempool-entries.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-mempool-entry.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-metrics.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-peer-addresses.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-server-info.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-sink-blue-score.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-sink.mdx` | `VERIFIED` |
| `content/docs/rpc/wrpc/get-subnetwork.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-sync-status.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-system-info.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-utxos-by-addresses.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/get-virtual-chain-from-block.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/ping.mdx` | `UNVERIFIED` |
| `content/docs/rpc/wrpc/resolve-finality-conflict.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/shutdown.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/submit-block.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/submit-transaction-replacement.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/submit-transaction.mdx` | `ISSUES` |
| `content/docs/rpc/wrpc/unban.mdx` | `ISSUES` |
| `content/docs/sdks/community-sdks.mdx` | `UNVERIFIED` |
| `content/docs/sdks/overview.mdx` | `UNVERIFIED` |
| `content/docs/sdks/python-sdk.mdx` | `UNVERIFIED` |
| `content/docs/sdks/rust-crates.mdx` | `UNVERIFIED` |
| `content/docs/sdks/wasm-sdk.mdx` | `UNVERIFIED` |
| `content/docs/security/responsible-disclosure.mdx` | `ISSUES` |
| `content/docs/security/security-model.mdx` | `ISSUES` |
| `content/docs/security/staying-safe.mdx` | `ISSUES` |
| `content/docs/wallets/cli-wallet.mdx` | `ISSUES` |
| `content/docs/wallets/hardware-wallets.mdx` | `UNVERIFIED` |
| `content/docs/wallets/hd-derivation.mdx` | `ISSUES` |
| `content/docs/wallets/overview.mdx` | `ISSUES` |
| `content/docs/wallets/wallet-api.mdx` | `ISSUES` |
| `content/docs/wallets/wallet-sdk.mdx` | `ISSUES` |
| `content/docs/wallets/web-wallet.mdx` | `UNVERIFIED` |
